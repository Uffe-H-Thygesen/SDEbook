...
## Analytical expression for the impulse response
x <- function(t) P/M / omega * exp(lambda*t)*sin(omega*t)

## Time range for plotting the impulse response
...
## Frequency range for frequency responses
omegas <- seq(omega/5,5*omega,omega/100)[-1]

pdf("frequency-response.pdf",width=3.5,height=3.5)
## Analytical expressions for the two frequency responses
...
## Analytical expression for the a.c.f. of the force
...
for(i in 2:N+1)
  {
    ...
    X[i] <- X[i-1]+V[i]*dt   ## Note: Symplectic time stepping
  }
...
## Analytical results for power spectra
omegas <- seq(1/tau/100,2.5*max(c(omega,1/tau)),omega/100)[-1]
Hxs <- F0/(K+C*1i*omegas - M*omegas^2)            ## Freq.response
Hvs <- F0*1i*omegas/(K+C*1i*omegas - M*omegas^2)
Sff <- 2*tau/(1+omegas^2*tau^2)                   ## Spectrum of force
Sxx <- Sff * Mod(Hxs)^2                           ## SPectrum of position 
Svv <- Sff * Mod(Hvs)^2                           ## SPectrum of velocity
...
## Plot spectrum of position; first analytical, then add empirical
plot(omegas,Sxx,type="l",log="xy",xaxt="n",
     xlab="",main="Variance spectra",
     ylab="")
title(ylab=expression("Position "*S[xx]*"[m"^2*s*"]"),line=4.5)

## Smooth the empirical spectrum in frequency bins
fs <- cut(log(XVFs$freq+0.1),100)
points(2*pi*exp(tapply(log(XVFs$freq),fs,mean)),
       2*exp(tapply(log(XVFs$spec[,1]),fs,mean)),pch=16)
grid()

## Plot spectrum of velocity; first analytical, then add empirical
par(mar=c(0,6,1,1))
...
## Smooth the empirical spectrum in frequency bins
points(2*pi*exp(tapply(log(XVFs$freq),fs,mean)),
       2*exp(tapply(log(XVFs$spec[,2]),fs,mean)),pch=16)
grid()

## Plot spectrum of force; first analytical, then add empirical
par(mar=c(5,6,1,1))
...
## Smooth the empirical spectrum in frequency bins
points(2*pi*exp(tapply(log(XVFs$freq),fs,mean)),
       2*exp(tapply(log(XVFs$spec[,3]),fs,mean)),pch=16)
grid()
dev.off()
