...
## Stationary density (unnornmalized)
phi <- Null(G)

## Normalize (this requires an equidistant grid)
phi <- phi / sum(phi*diff(xi))
...
CRED <- t(apply(PHI*dx,1,pdf2cred))
CDF <- t(apply(PHI*dx,1,cumsum))
